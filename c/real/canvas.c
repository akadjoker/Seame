...
void canvas_blt(Canvas *canvas, char *buffer, int x, int y, int w, int h)
{
    ...
    for (int i = 0; i < h; i++)
    {
        for (int j = 0; j < w; j++)
        {
            canvas_pixel(canvas, x + j, y + i, buffer[i * w + j]);
        }
    }
}
...
uint8_t *load_bmp_to_buffer(const char *filename, int *width, int *height)
{
    ...
    if (surface == NULL)
    {
        printf("Fail to load imagem %s! SDL_Error: %s\n", filename, SDL_GetError());
        return NULL;
    }
    ...
    *height = surface->h;
    ...
    // buffer para a imagem monocromática ///precisa ser FREE
    uint8_t *buffer = (uint8_t *)malloc((*width * *height) / 8 + 1);
    if (buffer == NULL)
    {
        SDL_FreeSurface(surface);
        return NULL;
    }
    memset(buffer, 0, (*width * *height) / 8 + 1);
    ...
    for (int y = 0; y < *height; y++)
    {
        for (int x = 0; x < *width; x++)
        {
            ...
            //  bit correspondente no buffer
            int byte_idx = (y * *width + x) / 8;
            int bit_idx = 7 - ((y * *width + x) % 8);

            if (gray > 127) // Threshold para preto/branco
            {
...
static void create_valid_variable_name(const char *input, char *output)
{
    int i = 0, j = 0;
